...
public class MainActivity extends AppCompatActivity {
    ...
    private byte _slaveAddress = (byte) 3;
    private I2CM
    ...
    public void sendButtonClicked(View view) {
        // Obter o valor do campo de texto inputSend
        EditText inputSendEditText = findViewById(R.id.inputSend);
        String inputSendText = inputSendEditText.getText().toString();

        _i2cManager.open();


        // Verificar se o campo de texto não está vazio
        if (inputSendText.isEmpty()) {
            Toast.makeText(this, "Please enter a value to send", Toast.LENGTH_SHORT).show();
            return;
        }

        // Converter o texto para byte e enviar para o dispositivo I²C
        byte valueToSend = Byte.parseByte(inputSendText);
